...
ОБЯЗАТЕЛЬНО про теги: если эксперт описывает ЛЮБОЕ повреждение,
дефект, замечание или след эксплуатации (царапина, скол, вмятина,
коррозия, потёртость, прожог/прожжённое место, пятно, разрыв, порез,
загрязнение, неработающий элемент, посторонний звук и т.п.) — для
каждого такого упоминания обязан появиться отдельный тег в seriousTags
или nonSeriousTags. Не объединяй разные дефекты в один тег. Синонимы
считай повреждениями (например: «притёртость»/«потёртость»/«затир» =
потёртость; «царапка» = царапина; «вмятинка» = вмятина; «ржа» =
ржавчина; «крашена»/«перекрашен» = покраска; «прожжено сигаретой»/
«след прожога»/«дырка от сигареты» = прожог).
Сначала ищи подходящий тег в каталоге раздела (точное совпадение по
смыслу, не по буквам — учитывай синонимы и формы слова). Если в
каталоге нет — ПРИДУМАЙ короткое имя (1–3 слова, по-русски, с
заглавной буквы, без точек) и положи в нужный массив. Пустые массивы
тегов допустимы ТОЛЬКО если noDamage=true.
...
4) Сформулируй заметку 1–2 предложения по делу: что видно, где, насколько критично.
   ЕСЛИ конкретный элемент по фото распознать НЕВОЗМОЖНО (общий план,
   нечитаемо, плохой ракурс) — ставь elementId="generalCondition" и в note
   опиши ОБЩЕЕ СОСТОЯНИЕ раздела «${sectionLabel}» по тому, что видно
   (например: «Кузов в целом — без явных следов ДТП, ЛКП ровное, мелкие
   сколы по передней части» / «Салон в целом — чистый, износ умеренный,
   видимых повреждений нет»). Не выдумывай конкретный элемент, если его
...
3) Для каждого тега реши, серьёзный это дефект или нет:
   ...
   ПРИОРИТЕТ — слова эксперта: если в заметке ЯВНО сказано
   «серьёзная/серьезная», «сильная», «глубокая», «большая», «крупная»,
   «значительная» — тег обязан попасть в seriousTags, даже если по
   умолчанию (вмятина/царапина и т.п.) он был бы не серьёзным. Слова
   «мелкая», «маленькая», «поверхностная», «лёгкая» — в nonSeriousTags.
   Сами слова-маркеры серьёзности в имя тега НЕ включай: имя оставляй
   коротким (тег «Вмятина», а не «Серьёзная вмятина»).
