...
Эксперт надиктовал заметку. Обработай её:
1) Определи elementId — какой именно элемент раздела имеет в виду эксперт.
   Используй подсказки-референсы (после ↳). Если заметка про несколько
   элементов сразу или общая — ставь "generalCondition".
   Если эксперт явно указывает другой элемент чем "текущий выбранный" —
   меняй elementId на тот, о котором он говорит.
2) Подбери подходящие теги из каталога (точное совпадение по смыслу). Если
   подходящего тега нет — придумай короткое имя (1–3 слова).
3) Реши, серьёзный это дефект или нет.
   Серьёзные: коррозия сквозная, перекрас, замена силового элемента,
   следы ДТП, треснутый элемент. Не серьёзные: сколы, царапины, локальные
   подкрасы, потёртости, грязь.
4) Переформулируй заметку 1–2 предложениями: что, где, насколько критично.
