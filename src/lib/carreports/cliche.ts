...
Эксперт надиктовал заметку текстом (БЕЗ фото). Твоя работа:
...
3) Для каждого тега реши, серьёзный это дефект или нет:
   ...
   • не серьёзные: сколы, царапины, локальные подкрасы, потёртости, грязь,
     мелкие вмятины без повреждения ЛКП.
...
4) Переформулируй заметку 1–2 короткими предложениями делового тона:
   ...
   из оригинала, ничего не додумывай и не выбрасывай.
...
Верни ТОЛЬКО валидный JSON одного объекта, без текста вокруг:
{
  "elementId": "<id из списка>",
  "noDamage": true|false,
  "seriousTags": ["имя_тега", ...],
