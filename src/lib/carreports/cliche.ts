...
export const CLICHE_INSPECTION_NOTE = (
  ...
  const tagList = knownTags.length
    ? knownTags
        ...
        .join("\n")
    : "  (каталог пуст — придумай короткие имена сам)";
  const elLine = elementLabel
    ? `Элемент: «${elementLabel}» (раздел «${sectionLabel}»).`
    : `Раздел: «${sectionLabel}».`;
  return `${COMMON}

${elLine}
Текст заметки эксперта ниже. Обработай его:
1) Подбери подходящие теги из каталога (точное совпадение по смыслу). Если
   подходящего тега нет — придумай короткое имя (1–3 слова).
2) Реши, серьёзный это дефект или нет.
...
3) Переформулируй заметку 1–2 предложениями: что, где, насколько критично.
