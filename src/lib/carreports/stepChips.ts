...
export const STEP_INTROS: Record<StepId, StepIntro> = {
  ...
  docs: {
    ...
    chips: [
      ...
      { label: "Да", value: "Собственник совпадает с ПТС/СТС.", group: "owner_match", single: true, groupLabel: "Собственник совпадает с продавцом?", groupKind: "yesno" },
      { label: "Нет", value: "Собственник НЕ совпадает с ПТС/СТС.", group: "owner_match", single: true, groupLabel: "Собственник совпадает с продавцом?", groupKind: "yesno" },
      { label: "Да", value: "VIN на кузове совпадает с документами.", group: "vin_match", single: true, groupLabel: "VIN на кузове совпадает с ПТС/СТС?", groupKind: "yesno" },
      { label: "Нет", value: "VIN на кузове НЕ совпадает с документами.", group: "vin_match", single: true, groupLabel: "VIN на кузове совпадает с ПТС/СТС?", groupKind: "yesno" },
      { label: "Да", value: "Номер двигателя совпадает с ПТС.", group: "engine_match", single: true, groupLabel: "Номер двигателя совпадает с ПТС?", groupKind: "yesno" },
      { label: "Нет", value: "Номер двигателя НЕ совпадает с ПТС.", group: "engine_match", single: true, groupLabel: "Номер двигателя совпадает с ПТС?", groupKind: "yesno" },
    ],
  },
  inspection: {
    greeting:
      "Перейдём к осмотру. Выберите раздел кузова ниже — я открою его и предложу загрузить фото. Каждое фото можно подписать заметкой, я сам распознаю элемент, серьёзность и подберу теги.",
    chips: [],
  },
  legalMaterials: {
    greeting:
      "Дополнительные материалы проверки. Прикрепите любые подтверждающие файлы: фото, видео, отчёты сканеров (PDF/DOC), скриншоты диагностики. Поддерживаются jpg/png/heic/webp, mp4/mov/avi, pdf/doc/docx. Когда добавите всё нужное — нажмите «Всё верно, далее».",
    chips: [],
  },
